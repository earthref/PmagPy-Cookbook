...
click on  the `Package Manager' button on the Canopy welcome page, and then on `Available Packages'.  Install both basemap packages (one has high resolution data files in it).  To install the Etopo20 package, see \href{#install_etopo.py}{install\_etopo.py}.
 \item Move the file Datafiles_2.0.zip from your {\bf PmagPy} folder and place it somewhere (e.g., your Desktop) with no spaces in the path.  Double click it  (for Macs) or `extract' the data files on a PC.  
...
\item  Choose Ôgeneric formatÕ and press [import file] button.  A new dialog box will appear. You can see  details about the generic file format by clicking  on the [help] button, and scrolling down to `INPUT'. In this dialog box,  fill in all the required information as shown here for AF or thermal demagnetization data:
...
\item After converting all  files to MagIC, press the [Next Step] Button. Press the [add all files with Ô.magicÕ suffix] . You should see a list of the three magic files:
...
\item An empty template of a file named ÔSrExample\_orient.txtÕ was created in the MagIC Project Directory. This file is displayed in a Python window. You can fill in  this file manually using the Python window or \href{#field_info}{with with a spreadsheet program} (recommended). The empty template has now only the sample names derived from your measurement files, using the naming rules that you chose.
...
There are several other tricks shown off in the notebook, which should be enough to get you started using {\bf IPmag} in a Python notebook environment.   
Conducting data analysis   using {\bf PmagPy}  in a notebook allows for the underlying code of statistical tests to be available and for the decisions made in the specific implementation of such tests to be transparently presented.  
