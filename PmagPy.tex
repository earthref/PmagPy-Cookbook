...
\item  Windows users should install this  Enthought Distribution: 
 ...
 \item Download the zip file from the  \href{https://github.com/ltauxe/PmagPy}{PmagPy} software package github: unzip the zip file (double click on it), PC users should extract all the files to a folder on your desktop (or wherever)  and double click on the {\it install\_MacOS} or {\it install\_Windows} file depending on your operating system.  (If you are a Linux user, just copy the files to some directory and put it in your path - you know what to do.)   For Windows users, the install process will create a custom ``PmagPy Prompt'' with a shortcut on your desktop.  This prompt is identical to the regular Command Prompt, except that it has all of the PmagPy functionality built in. Whenever the documentation refers to your ``command line'', you should use the PmagPy Prompt. For non-Windows users, find your \href{#command_line}{command line} and type {\bf eqarea.py -h} and hit return.  If you do not see the help message, first try setting your path by hand following the instructions \href{http://magician.ucsd.edu/Software/PmagPy/set_path.html}{here}.   If that fails, please send the error message and  Canopy version number to \href{mailto:ltauxe@ucsd.edu?subject=PmagPy Cookbook}{ltauxe@ucsd.edu}.  
  \customlink{hires}
...
click on  the `Package Manager' button on the Canopy welcome page, and then on `Available Packages'.  Install both basemap packages (one has high resolution data files in it).  To install the Etopo20 package, see \href{#install_etopo.py}{install\_etopo.py}.
 \item Move the file Datafiles\_2.0.zip from your {\bf PmagPy} folder and place it somewhere (e.g., your Desktop) with no spaces in the path.  Double click it  (for Macs) or `extract' the data files on a PC.  

  \item Create a \href{#Project_Directory}{MagIC Project Directory} (see Section~\ref{sect:ProjectDirectory}).
  \item Find your \href{#command_line}{command line} and type {\bf QuickMagIC.py}. 
...
\item Create a directory with a name that relates to that study.(e.g., ThisProject). The project directory name should have NO SPACES and be placed on the hard drive in a place that has NO spaces in the path.
Inside ThisProject directory, create two additional directories: MyFiles and MagIC.

\item Copy to your MyFiles folder the four files that are in /Datafiles\_2.0/QuickMagIC/QuickMagIC-upload/MyFiles.

\includegraphics[width=15cm]{EPSfiles/FigMyFiles.pdf}
...
\item In the MyFiles folder there are four files with AF, Thermal, and Thellier measurement data of samples from site sr01 (a lava flow site), location Snake River.
...
\item  Choose Ã”generic formatÃ• and press [import file] button.  A new dialog box will appear. You can see  details about the generic file format by clicking  on the [help] button, and scrolling down to `INPUT'. In this dialog box,  fill in all the required information as shown here for AF or thermal demagnetization data:
...
\item After converting all  files to MagIC, press the [Next Step] Button. Press the [add all files with Ã”.magicÃ• suffix] . You should see a list of the three magic files:
...
\item Step 5: fill in information for locations.  If you have provided site latitudes and longitudes, the columns for beginning and ending latitudes and longitudes should be filled in already. You must choose a legal entry from the controlled vocabularies for the  Òlocation\_typeÓ. In this example: location\_type = outcrop.

\includegraphics[width=32cm] {EPSfiles/ER_Builder_Step5.eps}

\item Step 6: fill in columns with any necessary information.  This is the last step. In this example: age=3.4; age\_sigma=0.03 (not a default header); age\_unit=Ma (from controlled vocabularies), magic\_method\_codes  = GM-ARAR-AP (from controlled vocabularies). It means "40Ar/39Ar age determination: Age plateau.Ó
...
For this, you just click the green {\it prepare upload txt file} button on the main page of the GUI. A file name `upload.txt' will be created in MagIC Project Directory.  Go to the  \href{http://earthref.org/MAGIC/search}{MagIC search interface}.      Click on the `create' button and find or create your reference. 

\includegraphics[width=15cm]{EPSfiles/upload2MagIC.eps}

Click on `upload' in the Actions column and   drag and drop the {\it upload.txt}   file onto the  `Drop a MagIC Text File' window. 
Congratulations. Your data are now in the database but are not yet activated.  Once you activate it (only for published papers), it will be publicly available.   
...
\item SIO format:   This option will generate a command line call to the program \href{#sio_magic.py}{sio\_magic.py}.  There are many options for laboratory procedures, naming conventions and so on and the {\bf QuickMagIC.py} GUI attempts to walk you through the process with as little pain as possible.  
	...
	\item Fill in the following information:  Ylou should really really fill in the location name for this file - it will save you time later if you want to upload your data into the MagIC database.  If your specimen/sample relationship is simply to add some characters to te end of your sample name (for example specimen mc144a1 belongs to sample mc144a), fill in the number of characters you use (unless it is '1' as in this case, because that is the default).  If you have some complicated system (for example specimen `\@\#\#\$\%\&\%` belongs to sample `Harry', we can't help you here and you'll have to fill out all the sample names later by hand.  (That should teach you a lesson).   
...
The program {\bf thellier\_gui.py}  combines functions from \href{#thellier_magic.py}{thellier\_magic.py} and new tools described by  \href{http://magician.ucsd.edu/~ltauxe/CV/open/shaar12.pdf}{Shaar and Tauxe (2012)} in a user-friendly graphical user interface (GUI).  It can also be called from within \href{#QuickMagIC.py}{QuickMagIC.py}, using files already prepared in the MagIC Project Directory and the interpretations from {\bf thellier\_gui.py} are part of the workflow of  \href{#QuickMagIC.py}.   This section is a brief introduction on how to use {\bf thellier\_gui.py} as a stand alone application.  For complete documentation, see:
...
A ``choose project directory'' dialog window will appear as soon as the GUI is started.
 ...
 The \href{#Project_Directory}{Project Directory} should include a file named ``magic\_measurements.txt'' .  To make this use the \href{#QuickMagIC.py} {QuickMagIC.py} GUI to convert your data files to the MagIC format.  Also, if a file named "rmag\_anistropy.txt" exists, then the program reads the anisotropy data from {\it rmag\_anistropy.txt }. Reading and processing the measurements files may take several seconds, depending on the number of the specimens.
...
Under the Windows operating system, if you followed the instructions in the  \href{#quick_start}{Quick Start} section, you will have a shortcut to the PmagPy command window on your desktop.  Double clicking on that will give you a window in which you can type {\bf PmagPy} commands.  
