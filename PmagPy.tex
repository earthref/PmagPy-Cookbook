...
\item To see an example of a filled in one, click on the ``Import Orientation File" button and  choose the file SrExample\_orient.txt from {\it MyFiles} folder.   
\item Click the button ``Save Orientation File".
\item Click the button ``Calculate Sample Orientations".
...
Notebooks are constructed as a series of `cells' which can be text or code.  To view the `source' of a text cell, just click on it.  To render it, click on the `run' button (sideways triangle on the toolbar) or type Shift+Return.  Similarly, to run the code in a code cell, click on the cell and then the `run' button or the keyboard shortcut. To execute the entire notebook, click on the `Cell' button and choose 'Run All'. 

The first step to using PmagPy functions within the notebook is to let it know where you put all the {\bf PmagPy} code. The default location for this on MacOS is in a folder called {\it PmagPy} in your home directory (e.g., /Users/YOUR\_NAME\_HERE/PmagPy) and on Windows is in the Documents folder (e.g., 'C:\\Users\\YOUR\_NAME\_HERE\\Documents\\PmagPy'. You must modify the line `sys.path.insert(0, `/Users/......') to reflect your own path.  After that modification, run the first code block which imports the necessary {\bf PmagPy} modules. Now the {\bf PmagPy} modules are available to use within the notebook.  The next  code block  imports several Python modules useful for scientific computing (numpy),  data manipulation (pandas) and plotting (matplotlib). Run this code block to make these functions available for use within the notebook.  
...
There are several other tricks shown off in the notebook, which should be enough to get you started using {\bf ipmag} in an IPython notebook environment. More of the functionality possible within the notebook can be seen at in this notebook: \url{http://nbviewer.ipython.org/github/Swanson-Hysell/PmagPy_IPython/blob/master/IPmag_examples.ipynb}. This notebook can also be downloaded from this repository so that you can run it locally on your own computer: \url{https://github.com/Swanson-Hysell/PmagPy_IPython}. Conducting data analysis using {\bf PmagPy} in a notebook allows for the underlying code of statistical tests to be available and for the decisions made in the specific implementation of such tests to be transparently presented.  
